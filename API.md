...
To make a request to a postbox via Tor (running with a SOCKS proxy on the system already),
...
## Postbox Management (owner only)


### Setup/Pairing (owner only)

Used by owner to pair with or setup their postbox (which needs to be in the setup state).
Request is authenticated by a single-use token from the QR code shown by the postbox app.
...
a list of API versions that the Postbox supports. Response example:
...
Retrieves the list of API versions that the Postbox supports.
...
Checks whether postbox is running.
...
Adds a new contact to the postbox.
...
Gets list of contacts managed by the postbox identified by their `contactId`.
...
The postbox checks if the provided auth token is allowed to upload to $folderId
...
The postbox chooses a random string for the file ID (32 hex encoded bytes).
...
The postbox checks if provided auth token is allowed to download from $folderId
which is either an `inboxId` or an `outboxId` of a contact.
